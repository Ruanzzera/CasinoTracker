...
begin
  ...
  if v_balance <> 0 then
    insert into public.casino_entries(user_id, amount, type, house, game, notes, created_at)
    values (v_user, v_balance, 'aposte_ganhe', p_house, v_dash_game,
            'Aposte e Ganhe — ' || p_house, p_created_at)
  ...
  insert into public.bet_and_win_entries(
    ...
    spin_count, spin_bet, spin_prize, casino_entry_id
  ) values (
    ...
    p_spin_count, p_spin_bet, p_spin_prize, v_casino_id
  ) returning * into v_row;

  return v_row;
end $$;

grant execute on function public.bet_and_win_create(date,text,text,text,text,numeric,numeric,numeric,numeric,int,numeric,numeric,timestamptz) to authenticated;

-- Atomic update (with orphan-linking fallback for legacy data)
...
grant execute on function public.bet_and_win_update(uuid,date,text,text,text,text,numeric,numeric,numeric,numeric,int,numeric,numeric,timestamptz) to authenticated;

-- Atomic delete
create or replace function public.bet_and_win_delete(p_id uuid)
returns void
language plpgsql security definer set search_path = public as $$
declare
  v_user uuid := auth.uid();
...
begin
  if v_user is null then raise exception 'not authenticated'; end if;
  select casino_entry_id into v_casino from public.bet_and_win_entries
    where id = p_id and user_id = v_user;
  if v_casino is not null then
    delete from public.casino_entries where id = v_casino and user_id = v_user;
  end if;
  delete from public.bet_and_win_entries where id = p_id and user_id = v_user;
end $$;

grant execute on function public.bet_and_win_delete(uuid) to authenticated;
